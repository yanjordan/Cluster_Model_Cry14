...
CONTAINS
        ...
        DOUBLEPRECISION FUNCTION norm_cle(alpha, n)
		IMPLICIT NONE
                DOUBLEPRECISION, INTENT(in) :: alpha
                INTEGER, INTENT(in) :: n
                norm_cle=0.d0
                
                norm_cle=sqrt((2*alpha)**(2*n+1) /fact(2*n))

        
        ENDFUNCTION

        DOUBLEPRECISION FUNCTION Den_H(r)
		IMPLICIT NONE
                DOUBLEPRECISION, INTENT(in) :: r
                Den_H=0.d0
                
!                Den_H=1d0*(norm_cle(1d0,1)*1d0*exp(-1d0*r))**2
                Den_H=4*exp(-2*r)
                
                
        ENDFUNCTION
        
        DOUBLEPRECISION FUNCTION Den_C(r)
		IMPLICIT NONE
                DOUBLEPRECISION, INTENT(in) :: r
                DOUBLEPRECISION :: wf
                ...
                wf=0.d0
                do i=1,6
                        wf=wf+norm_cle(alphas(i),sn(i))*r**(sn(i)-1)*sc1(i)*exp(-alphas(i)*r)
                ENDDO
                ...
                wf=0.d0
                do i=1,6
                        wf=wf+norm_cle(alphas(i),sn(i))*r**(sn(i)-1)*sc2(i)*exp(-alphas(i)*r)
                ENDDO
                ...
                wf=0.d0
                do i=1,4
                        wf=wf+norm_cle(alphap(i),pn(i))*r**(pn(i)-1)*pc1(i)*exp(-alphap(i)*r)
                ENDDO
                Den_C=Den_C+2*wf**2
        ENDFUNCTION
        
        DOUBLEPRECISION FUNCTION Den_N(r)
		IMPLICIT NONE
                DOUBLEPRECISION, INTENT(in) :: r
                DOUBLEPRECISION :: wf
        ...
        DOUBLEPRECISION FUNCTION Den_O(r)
		IMPLICIT NONE
                DOUBLEPRECISION, INTENT(in) :: r
                DOUBLEPRECISION :: wf
        ...
        DOUBLEPRECISION FUNCTION Den_S(r)
		IMPLICIT NONE
                DOUBLEPRECISION, INTENT(in) :: r
                DOUBLEPRECISION :: wf
                ...
                wf=0.d0
                do i=1,8
                        wf=wf+norm_cle(alphas(i),sn(i))*r**(sn(i)-1)*sc3(i)*exp(-alphas(i)*r)
                ENDDO
                ...
                Den_S=Den_S+6*wf**2
                 ...
                 wf=0.d0
                do i=1,8
                        wf=wf+norm_cle(alphap(i),pn(i))*r**(pn(i)-1)*pc2(i)*exp(-alphap(i)*r)
                ENDDO
                Den_S=Den_S+4*wf**2
        ENDFUNCTION

        DOUBLEPRECISION FUNCTION Den_Y(r)
		IMPLICIT NONE
                DOUBLEPRECISION, INTENT(in) :: r
                DOUBLEPRECISION :: wf
...
! s orbital                
                ...
                wf=0.d0
                do i=1,13
                        wf=wf+norm_cle(alphas(i),sn(i))*r**(sn(i)-1)*sc4(i)*exp(-alphas(i)*r)
                ENDDO
                ...
                wf=0.d0
                do i=1,13
                        wf=wf+norm_cle(alphas(i),sn(i))*r**(sn(i)-1)*sc5(i)*exp(-alphas(i)*r)
                ENDDO
...
! p orbital                
                ...
                wf=0.d0
                do i=1,10
                        wf=wf+norm_cle(alphap(i),pn(i))*r**(pn(i)-1)*pc3(i)*exp(-alphap(i)*r)
                ENDDO
                Den_Y=Den_Y+6*wf**2
! d orbital                
                wf=0.d0
                do i=1,8
                        wf=wf+norm_cle(alphad(i),dn(i))*r**(dn(i)-1)*dc1(i)*exp(-alphad(i)*r)
                ENDDO
                Den_Y=Den_Y+10*wf**2
                
                wf=0.d0
                do i=1,8
                        wf=wf+norm_cle(alphad(i),dn(i))*r**(dn(i)-1)*dc2(i)*exp(-alphad(i)*r)
                ENDDO
                Den_Y=Den_Y+1*wf**2
        ...
        DOUBLEPRECISION FUNCTION Den_Ti(r)
		IMPLICIT NONE
                DOUBLEPRECISION, INTENT(in) :: r
                DOUBLEPRECISION :: wf
...
! d orbital                
	...
	DOUBLEPRECISION FUNCTION Cle_DEN(r)
	        IMPLICIT NONE
	        
		DOUBLEPRECISION, DIMENSION(3) :: r
		DOUBLEPRECISION :: dis_r
                       ...
                       do i= -1,1
						...
						do m=1,num_atom_cell
							...
							if (trim(atome_ele_list(m))=='H') then
								Cle_DEN=Cle_DEN+Den_H(dis_r)/(4*pi)
							elseif (trim(atome_ele_list(m))=='C') then
								Cle_DEN=Cle_DEN+Den_C(dis_r)/(4*pi)
							elseif (trim(atome_ele_list(m))=='N') then
								Cle_DEN=Cle_DEN+Den_N(dis_r)/(4*pi)
							elseif (trim(atome_ele_list(m))=='O') then
								Cle_DEN=Cle_DEN+Den_O(dis_r)/(4*pi)
							elseif (trim(atome_ele_list(m))=='S') then
								Cle_DEN=Cle_DEN+Den_S(dis_r)/(4*pi)
							elseif (trim(atome_ele_list(m))=='TI') then
								Cle_DEN=Cle_DEN+Den_Ti(dis_r)/(4*pi)
				                        elseif (trim(atome_ele_list(m))=='Y') then
								Cle_DEN=Cle_DEN+Den_Y(dis_r)/(4*pi)
							endif
						enddo
                                        ENDDO
                                ENDDO
